...
```{r}
# Rename Primary Care Physician Rate to have no spaces (for backwards regression function)
merged_data <- merged_data  %>% 
  dplyr::rename(
    Primary_Care_Physicians_Rate = `Primary Care Physicians Rate`
  )

# Fit Full Linear Regression Model
full_mod = lm(Primary_Care_Physicians_Rate ~ 
    total_population +
    housing_units +
    occupied_households +
    socioeconomic_score +
...
# Predicted PCP rate using full model accuracy
merged_data$pred_full <- predict(full_mod)

ggplot(merged_data,
       aes(x = pred_full, y = Primary_Care_Physicians_Rate)) +
...
# Plotting PCP rate based on SVI index
ggplot(merged_data, aes(x = x2018_overall_svi_score, y = Primary_Care_Physicians_Rate)) +
  geom_point(alpha = 0.6, color = "steelblue") +
  geom_smooth(method = "lm", se = TRUE, color = "darkred") +
  geom_text_repel(data = subset(merged_data, Primary_Care_Physicians_Rate > quantile(Primary_Care_Physicians_Rate, 0.95)),
...
```{r}
# Backwards Stepwise Regression Using p-value as goodness of fit criteria, cutoff 0.05
step_mod <- ols_step_backward_p(full_mod, prem = 0.05)
step_mod
```

# 
```{r}
final_mod <- step_mod$model
summary(final_mod)
```
